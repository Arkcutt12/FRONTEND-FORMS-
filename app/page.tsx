  ...
  return (
    <>
      <div className="w-full h-screen bg-white flex flex-col">
        <div className="w-full bg-[#FAFAFA] overflow-hidden flex-1 flex flex-col">
          {/* Show Order Summary */}
          {showOrderSummary && (
            <OrderSummary
              formData={formData}
          ...
          {!showOrderSummary && !showRequestSuccess && (
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              {/* Left Side - File Upload Area or DXF Viewer */}
              <div className="flex-1 bg-[#FAFAFA] relative overflow-hidden">
                {previewFile ? (
                  <DXFViewer file={previewFile} onClose={closePreview} />
                ) : (
                  <FileUploadArea onUploadClick={handleUploadClick} hasFiles={formData.files.length > 0} />
                )}
                <input
                  type="file"
                  ref={fileInputRef}
